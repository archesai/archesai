...
`,
	...
	p := NewParser().WithFS(fsys)

	s, err := p.Parse("openapi.yaml")
	if err != nil {
...
`,
	...
	fsys := makeTestFS(files)
	p := NewParser().WithFS(fsys).WithIncludes([]string{"server"})
...
func TestParser_Parse_DefaultValue(t *testing.T) {
	parser := NewParser()
	s, err := parser.Parse("../../pkg/auth/api/openapi.yaml")
	if err != nil {
		t.Fatal(err)
	}
		...
		if schema.Name == "APIKey" {
			for propName, propRef := range schema.Properties {
				if propName == "Scopes" {
					...
					// Check that Default is []interface{} (same as []any)
					defaultSlice, ok := prop.Default.([]interface{})
					if !ok {
						t.Errorf("Scopes.Default type = %T, want []interface{}", prop.Default)
					}
