...
func Middleware(authService *core.Service, logger *slog.Logger) echo.MiddlewareFunc {
...
func OptionalAuthMiddleware(authService *core.Service, logger *slog.Logger) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			// Extract token from Authorization header
			token := extractToken(c)
...
func RequireRole(roles ...string) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			claims, ok := c.Get(string(ClaimsContextKey)).(*core.Claims)
			if !ok {
...
func GetClaimsFromContext(c echo.Context) (*core.Claims, bool) {
	claims, ok := c.Get(string(ClaimsContextKey)).(*core.Claims)
	return claims, ok
}
