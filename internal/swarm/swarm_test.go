...
func TestHandleToolCall(t *testing.T) {
	...
	agentFunction := AgentFunction{
		Name:        "testFunction",
		Description: "A test function",
		Function: func(_ map[string]interface{}, _ map[string]interface{}) Result {
			return Result{
				Success: true,
	...
	response := sw.handleToolCall(ctx, &toolCall, agent, contextVariables, false)

	assert.Len(t, response.Messages, 1)
	assert.Equal(t, llm.RoleFunction, response.Messages[0].Role)
	assert.Contains(t, response.Messages[0].Content, "Function executed successfully")
}
