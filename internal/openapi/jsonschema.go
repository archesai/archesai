// Package openapi provides OpenAPI and JSON Schema parsing utilities.
package openapi
...
func (p *JSONSchemaParser) ParseBase(schema *base.Schema) (*spec.Schema, error) {
	if schema == nil {
		return nil, fmt.Errorf("schema is nil")
	}

	// Extract schema type from extension
	schemaType := spec.XCodegenSchemaTypeValueobject
	if schema.Extensions != nil {
		if ext, ok := schema.Extensions.Get("x-codegen-schema-type"); ok {
			...
			if err := ext.Decode(&schemaTypeStr); err == nil {
				schemaType = spec.XCodegenSchemaType(schemaTypeStr)
			}
		}
	}
...
// extractSchemaRecursive builds a recursive Schema structure
func (p *JSONSchemaParser) extractSchemaRecursive(
	schema *base.Schema,
	parentName string,
	schemaName string,
	schemaType spec.XCodegenSchemaType,
) (*spec.Schema, error) {
	if schema == nil {
		return nil, fmt.Errorf("schema is nil")
	}

	result := p.buildBasicSchema(schema, schemaType)
...
// buildBasicSchema creates the basic Schema structure
func (p *JSONSchemaParser) buildBasicSchema(
	schema *base.Schema,
	schemaType spec.XCodegenSchemaType,
) *spec.Schema {
	result := &spec.Schema{
	...
	if schema.Extensions != nil {
		if ext, ok := schema.Extensions.Get("x-codegen"); ok {
			var xcodegen spec.XCodegenExtension
...
func (p *JSONSchemaParser) extractSchemaType(schema *base.Schema, result *spec.Schema) {
	if len(schema.Type) > 0 {
		...
		result.Type = spec.SchemaTypeObject
	}
...
func (p *JSONSchemaParser) processArrayItemInline(
	...
	if result.Items != nil && result.Items.Type == spec.SchemaTypeObject &&
...
func (p *JSONSchemaParser) processSimpleSchema(schema *base.Schema, result *spec.Schema) {
	if schema.Enum == nil {
		return
	}
...
func (p *JSONSchemaParser) resolvePropertyType(
	...
	currentSchemaType spec.XCodegenSchemaType,
	...
	targetPackage := string(spec.XCodegenSchemaTypeEntity) // Default to entity
	xInternal := ""
...
// extractPropertiesInto extracts properties into a parent Schema
	...
	for propPair := schema.Properties.First(); propPair != nil; propPair = propPair.Next() {
		...
		if refString := propProxy.GetReference(); refString != "" {
			...
			if strings.HasPrefix(refString, "#/components/schemas/") {
				...
				// Create a simple Schema for the reference
				refDef := &spec.Schema{
					Name:    fieldName,
					JSONTag: propName,
					YAMLTag: propName,
			...
			if propDef.Type == spec.SchemaTypeObject && typeName != "" && len(propDef.Properties) > 0 {
...
func (p *JSONSchemaParser) computeGoType(
	...
	return typeconv.SchemaToGoType(schema.Schema, p.openAPIDoc, string(currentSchemaType))
}
