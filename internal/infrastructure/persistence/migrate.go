...
func (m *MigrationRunner) Up() error {
	...
	case TypePostgreSQL:
		...
		// Create migration directory from embed.FS
		migrationsFS, err = fs.Sub(migrations, "postgres/migrations")
		if err != nil {
	...
	case TypeSQLite:
		...
		// Create migration directory from embed.FS
		migrationsFS, err = fs.Sub(sqliteMigrations, "sqlite/migrations")
		if err != nil {
			return fmt.Errorf("failed to create sub filesystem: %w", err)
		}
