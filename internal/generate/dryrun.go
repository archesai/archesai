...
func printDryRunResults(cg *codegen.Codegen, outputPath string) error {
...
func printDryRunResultsTUI(runner *tui.Runner, cg *codegen.Codegen, outputPath string) error {
	memStorage := cg.GetStorage().(*storage.MemoryStorage)
	files := memStorage.GetFiles()
