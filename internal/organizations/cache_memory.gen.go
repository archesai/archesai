...
func (c *MemoryCache) GetOrganization(ctx context.Context, id uuid.UUID) (*Organization, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	entity, ok := item.data.(*Organization)
	if !ok {
...
func (c *MemoryCache) SetOrganization(ctx context.Context, entity *Organization, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
