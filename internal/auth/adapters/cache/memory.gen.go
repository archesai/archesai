...
func NewMemoryCache() auth.Cache {
	maxItems := 1000
	ttl := 5 * time.Minute
...
func NewMemoryCacheWithOptions(maxItems int, ttl time.Duration) auth.Cache {
	cache := &MemoryCache{
		items:    make(map[string]*cacheItem),
		maxItems: maxItems,
		ttl:      ttl,
...
func (c *MemoryCache) GetAccount(ctx context.Context, id uuid.UUID) (*auth.Account, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	entity, ok := item.data.(*auth.Account)
	if !ok {
...
func (c *MemoryCache) SetAccount(ctx context.Context, entity *auth.Account, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
...
func (c *MemoryCache) GetSession(ctx context.Context, id uuid.UUID) (*auth.Session, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	entity, ok := item.data.(*auth.Session)
	if !ok {
...
func (c *MemoryCache) SetSession(ctx context.Context, entity *auth.Session, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
...
func (c *MemoryCache) GetSessionByToken(ctx context.Context, token string) (*auth.Session, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *MemoryCache) SetSessionByToken(ctx context.Context, token string, entity *auth.Session, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
