...
func NewMemoryCache() users.Cache {
	maxItems := 1000
	ttl := 5 * time.Minute
...
func NewMemoryCacheWithOptions(maxItems int, ttl time.Duration) users.Cache {
	cache := &MemoryCache{
		items:    make(map[string]*cacheItem),
		maxItems: maxItems,
		ttl:      ttl,
...
func (c *MemoryCache) GetUser(ctx context.Context, id uuid.UUID) (*users.User, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	entity, ok := item.data.(*users.User)
	if !ok {
...
func (c *MemoryCache) SetUser(ctx context.Context, entity *users.User, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
...
func (c *MemoryCache) GetUserByEmail(ctx context.Context, email string) (*users.User, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *MemoryCache) SetUserByEmail(ctx context.Context, email string, entity *users.User, ttl time.Duration) error {
	if entity == nil {
		return nil
	}
