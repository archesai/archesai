...
type ConstructorDef struct {
	...
	Parameters []spec.Param // Constructor parameters
}
...
func (r *DependencyRegistry) Resolve(typeName string) (DependencyResolution, bool) {
	...
	if strings.HasPrefix(typeName, "repositories.") && strings.HasSuffix(typeName, "Repository") {
		...
		return DependencyResolution{
			ImportPath: "", // Same package
			Resolution: "repos." + strutil.Pluralize(entityName),
		}, true
	}
...
func (p *HandlerParser) ParseHandlers(operations []spec.Operation) ([]HandlerDef, error) {
...
func (p *HandlerParser) parseHandler(op spec.Operation) (HandlerDef, error) {
...
func (p *HandlerParser) extractParameters(fn *ast.FuncDecl, file *ast.File) []spec.Param {
