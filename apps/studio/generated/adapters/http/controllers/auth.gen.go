...
import (
	...
	customHandlers "github.com/archesai/archesai/apps/studio/handlers/auth"
	"github.com/archesai/archesai/pkg/server"
)

// AuthController handles HTTP requests for auth endpoints.
type AuthController struct {
	confirmEmailChangeHandler       *customHandlers.ConfirmEmailChangeCommandHandler
	confirmEmailVerificationHandler *customHandlers.ConfirmEmailVerificationCommandHandler
	confirmPasswordResetHandler     *customHandlers.ConfirmPasswordResetCommandHandler
	linkAccountHandler              *customHandlers.LinkAccountCommandHandler
	loginHandler                    *customHandlers.LoginCommandHandler
	logoutHandler                   *customHandlers.LogoutCommandHandler
	logoutAllHandler                *customHandlers.LogoutAllCommandHandler
	registerHandler                 *customHandlers.RegisterCommandHandler
	requestEmailChangeHandler       *customHandlers.RequestEmailChangeCommandHandler
	requestEmailVerificationHandler *customHandlers.RequestEmailVerificationCommandHandler
	requestMagicLinkHandler         *customHandlers.RequestMagicLinkCommandHandler
	requestPasswordResetHandler     *customHandlers.RequestPasswordResetCommandHandler
	verifyMagicLinkHandler          *customHandlers.VerifyMagicLinkCommandHandler
	updateAccountHandler            *customHandlers.UpdateAccountCommandHandler
	updateSessionHandler            *customHandlers.UpdateSessionCommandHandler
	deleteAccountHandler            *customHandlers.DeleteAccountCommandHandler
	deleteSessionHandler            *customHandlers.DeleteSessionCommandHandler
	getAccountHandler               *queries.GetAccountQueryHandler
	getSessionHandler               *queries.GetSessionQueryHandler
	oauthAuthorizeHandler           *customHandlers.OAuthAuthorizeQueryHandler
	oauthCallbackHandler            *customHandlers.OAuthCallbackQueryHandler
...
type RequestMagicLink200Response struct {
	...
	Token     core.MagicLinkToken `json:"token,omitempty"`
}
...
func (c *AuthController) ListAccounts(w http.ResponseWriter, r *http.Request) {
	...
	data := make([]core.Account, len(results))
...
func (c *AuthController) ListSessions(w http.ResponseWriter, r *http.Request) {
	...
	// Convert pointer slice to value slice for response
	data := make([]core.Session, len(results))
	for i, item := range results {
		if item != nil {
			data[i] = *item
		}
