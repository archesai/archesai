...
func (r *PostgresRunRepository) Create(ctx context.Context, entity *schemas.Run) (*schemas.Run, error) {
...
func (r *PostgresRunRepository) Get(ctx context.Context, id uuid.UUID) (*schemas.Run, error) {
	params := GetRunParams{
		ID: id,
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || err == sql.ErrNoRows {
			return nil, schemas.ErrRunNotFound
		}
...
func (r *PostgresRunRepository) Update(ctx context.Context, id uuid.UUID, entity *schemas.Run) (*schemas.Run, error) {
...
func (r *PostgresRunRepository) List(ctx context.Context, limit, offset int32) ([]*schemas.Run, int64, error) {
	...
	items := make([]*schemas.Run, len(results))
...
func (r *PostgresRunRepository) ListRunsByPipeline(ctx context.Context, pipelineID string) ([]*schemas.Run, error) {
	...
	items := make([]*schemas.Run, len(result))
...
func (r *PostgresRunRepository) ListRunsByOrganization(ctx context.Context, organizationID string) ([]*schemas.Run, error) {
...
func (r *PostgresRunRepository) ListRunsByTool(ctx context.Context, toolID string) ([]*schemas.Run, error) {
...
func mapRunFromDB(db *Run) *schemas.Run {
	if db == nil {
		return nil
	}
	...
	result := &schemas.Run{
		...
		Status:         schemas.RunStatus(db.Status),
		ToolID:         db.ToolID,
	}
