...
func (r *PostgresPipelineRepository) Create(ctx context.Context, entity *schemas.Pipeline) (*schemas.Pipeline, error) {
...
func (r *PostgresPipelineRepository) Get(ctx context.Context, id uuid.UUID) (*schemas.Pipeline, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || err == sql.ErrNoRows {
			return nil, schemas.ErrPipelineNotFound
		}
...
func (r *PostgresPipelineRepository) Update(ctx context.Context, id uuid.UUID, entity *schemas.Pipeline) (*schemas.Pipeline, error) {
...
func (r *PostgresPipelineRepository) Delete(ctx context.Context, id uuid.UUID) error {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || err == sql.ErrNoRows {
			return schemas.ErrPipelineNotFound
		}
...
func (r *PostgresPipelineRepository) List(ctx context.Context, limit, offset int32) ([]*schemas.Pipeline, int64, error) {
	...
	items := make([]*schemas.Pipeline, len(results))
...
func (r *PostgresPipelineRepository) ListPipelinesByOrganization(ctx context.Context, organizationID string) ([]*schemas.Pipeline, error) {
	...
	items := make([]*schemas.Pipeline, len(result))
...
func mapPipelineFromDB(db *Pipeline) *schemas.Pipeline {
	if db == nil {
		return nil
	}

	result := &schemas.Pipeline{
		ID:             db.ID,
