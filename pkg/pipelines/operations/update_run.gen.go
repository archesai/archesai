// Code generated by archesai. DO NOT EDIT.

package operations

import (
	"context"
	"fmt"
	"time"

	"github.com/archesai/archesai/pkg/events"
	"github.com/archesai/archesai/pkg/executor"
	"github.com/archesai/archesai/pkg/pipelines/schemas"
	"github.com/google/uuid"
)

// ============================================================================
// UpdateRun
// ============================================================================

// UpdateRunInput represents the input for the UpdateRun operation.
type UpdateRunInput struct {
	ID         uuid.UUID
	PipelineID *uuid.UUID
}

// UpdateRunOutput represents the output for the UpdateRun operation.
type UpdateRunOutput struct {
	Data schemas.Run `json:"data"`
}

// UpdateRun defines the executor interface for the UpdateRun operation.
type UpdateRun = executor.Executor[UpdateRunInput, UpdateRunOutput]

// UpdateRunImpl is the default implementation of UpdateRun.
type UpdateRunImpl struct {
	repo      schemas.RunRepository
	publisher events.Publisher
}

// NewUpdateRun creates a new UpdateRun handler.
func NewUpdateRun(
	repo schemas.RunRepository,
	publisher events.Publisher,
) UpdateRun {
	return &UpdateRunImpl{
		repo:      repo,
		publisher: publisher,
	}
}

// Execute performs the UpdateRun operation.
func (h *UpdateRunImpl) Execute(ctx context.Context, input *UpdateRunInput) (*UpdateRunOutput, error) {
	// Get existing entity
	existing, err := h.repo.Get(ctx, input.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to get run: %w", err)
	}

	// Update fields
	// TODO: Map input fields to entity
	existing.UpdatedAt = time.Now().UTC()

	// Save to repository
	updated, err := h.repo.Update(ctx, input.ID, existing)
	if err != nil {
		return nil, fmt.Errorf("failed to update run: %w", err)
	}

	// Publish event
	event := schemas.NewRunUpdatedEvent(updated.ID)
	_ = h.publisher.Publish(ctx, event)

	// Map to output
	output := &UpdateRunOutput{
		// TODO: Map updated entity to output
	}
	_ = updated

	return output, nil
}
