...
export function OpenAPIViewer({ spec }: OpenAPIViewerProps): JSX.Element {
  ...
  const requestBodyEntries = sortInternalMarkerLast(Object.entries(requestBodies));
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <Tabs className="flex flex-1 flex-col" defaultValue="paths">
        <div className="overflow-x-auto border-b px-4">
          <TabsList className="w-max">
            <TabsTrigger value="paths">Paths ({pathEntries.length})</TabsTrigger>
            ...
            {linkEntries.length > 0 && (
              <TabsTrigger value="links">Links ({linkEntries.length})</TabsTrigger>
            )}
        ...
        <TabsContent className="m-0 flex-1" value="paths">
        ...
        <TabsContent className="m-0 flex-1" value="schemas">
          <ScrollArea className="h-[calc(100vh-220px)]">
            <div className="space-y-4 p-4">
        ...
        {responseEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="responses">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
        ...
        {requestBodyEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="requestBodies">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
        ...
        {parameterEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="parameters">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
                {parameterEntries.map(([name, parameter]) => (
                  <ParameterViewer key={name} name={name} parameter={parameter} />
                ))}
              </div>
            </ScrollArea>
          </TabsContent>
        ...
        {headerEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="headers">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
                {headerEntries.map(([name, header]) => (
                  <HeaderViewer header={header} key={name} name={name} />
                ))}
              </div>
            </ScrollArea>
          </TabsContent>
        ...
        {exampleEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="examples">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
                {exampleEntries.map(([name, example]) => (
                  <ExampleViewer example={example} key={name} name={name} />
                ))}
              </div>
            </ScrollArea>
          </TabsContent>
        ...
        {linkEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="links">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
                {linkEntries.map(([name, link]) => (
                  <LinkViewer key={name} link={link} name={name} />
                ))}
              </div>
            </ScrollArea>
          </TabsContent>
        ...
        {callbackEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="callbacks">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
        ...
        {securityEntries.length > 0 && (
          <TabsContent className="m-0 flex-1" value="security">
            <ScrollArea className="h-[calc(100vh-220px)]">
              <div className="space-y-4 p-4">
                {securityEntries.map(([name, scheme]) => (
                  <div className="rounded-lg border p-4" key={name}>
                    ...
                    <dl className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      {scheme.bearerFormat && (
                        <>
                          <dt className="text-muted-foreground">Bearer Format</dt>
                        ...
                        <>
                          ...
                          <dd>
                            {Object.keys(scheme.flows).map((flow) => (
                              <Badge className="mr-1" key={flow} variant="outline">
                                {flow}
                              </Badge>
                            ))}
                          </dd>
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        ...
        {internalValue && (
          <>
            <Badge className="bg-sky-500/10 text-sky-500 text-xs">internal</Badge>
          ...
          <div>
            ...
            <div className="space-y-2">
              {Object.entries(response.content).map(([contentType, mediaType]) => (
                <div className="flex items-center gap-2 text-sm" key={contentType}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {Object.entries(response.headers).map(([headerName, header]) => (
                <div className="flex items-start gap-2 text-sm" key={headerName}>
...
function ParameterViewer({ name, parameter }: ParameterViewerProps): JSX.Element {
      ...
      {parameter.schema &&
        ...
        parameter.schema.enum && (
          <div className="text-sm">
            ...
            {parameter.schema.enum.map((v, i) => (
              <code className="mx-1 rounded bg-muted px-1" key={i}>
                {String(v)}
              </code>
            ))}
          </div>
...
function HeaderViewer({ name, header }: HeaderViewerProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {header.description && (
        <p className="mb-3 text-muted-foreground text-sm">{header.description}</p>
      )}
...
function ExampleViewer({ name, example }: ExampleViewerProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      {example.description && (
        <p className="mb-3 text-muted-foreground text-sm">{example.description}</p>
      )}
      ...
      <div className="space-y-4">
        {Object.entries(callback).map(([expression, pathItem]) => (
          <div key={expression}>
            ...
            <PathViewer path={expression} pathItem={pathItem} schemas={schemas} />
          </div>
        ))}
      </div>
    </div>
