...
// MigrationsGenerator handles database migration generation using Atlas.
...
// NewMigrationsGenerator creates a new MigrationsGenerator.
func NewMigrationsGenerator(outputDir string) *MigrationsGenerator {
...
func (m *MigrationsGenerator) Start(ctx context.Context, dbType Type) error {
	switch dbType {
	case TypePostgreSQL:
		db, container, err := StartPostgreSQL(ctx)
		if err != nil {
...
func (m *MigrationsGenerator) Stop(ctx context.Context) error {
...
func (m *MigrationsGenerator) GenerateMigration(ctx context.Context) error {
	if m.database == nil {
	...
	case TypePostgreSQL:
		migrationDir = filepath.Join(m.outputDir, "database", "postgres", "migrations")
		schemaName = postgresSchemaName
	case TypeSQLite:
		migrationDir = filepath.Join(m.outputDir, "database", "sqlite", "migrations")
	...
	runner := NewMigrationRunner(m.database, migrationsFS)
	...
	currentSchema, err := driver.InspectSchema(ctx, schemaName, &schema.InspectOptions{
		// Exclude Atlas revision tracking table from schema inspection
...
`, packageName)
	...
	slog.Debug("Migration file created", slog.String("path", migrationPath))
	return nil
}

func (m *MigrationsGenerator) loadHCLSchema() (*schema.Schema, error) {
	var hclSchemaFile string
	switch m.database.Type() {
	case TypePostgreSQL:
		hclSchemaFile = filepath.Join(m.outputDir, "database", "postgres", "schema.gen.hcl")
	case TypeSQLite:
		hclSchemaFile = filepath.Join(m.outputDir, "database", "sqlite", "schema.gen.hcl")
	default:
...
func (m *MigrationsGenerator) getNextMigrationVersion(migrationDir string) int {
...
func (m *MigrationsGenerator) formatMigrationSQL(plan *migrate.Plan) string {
