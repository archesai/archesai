...
export class MembershipGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Observable<boolean> | Promise<boolean> {
    ...
    const membership = currentUser.memberships.find((val) => val.orgname == orgname)
    if (!membership) {
      this.logger.error(`User ${currentUser.username} is not a member of organization ${orgname}`)
    ...
    if (!roles.includes(membership.role)) {
      this.logger.error(`User ${currentUser.username} does not have the required role in organization ${orgname}`)
