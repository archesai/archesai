...
export interface StorageService {
  ...
  download(orgname: string, path: string, destination?: string): Promise<{ buffer: Buffer }>
  getMetaData(orgname: string, path: string): Promise<{ metadata: any }>
  getSignedUrl(orgname: string, path: string, action: 'read' | 'write'): Promise<string>
  listDirectory(orgname: string, path: string): Promise<StorageItemDto[]>
  upload(orgname: string, path: string, file: Express.Multer.File): Promise<string>
