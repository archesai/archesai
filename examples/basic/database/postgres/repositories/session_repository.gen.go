...
func (r *PostgresSessionRepository) Create(ctx context.Context, entity *schemas.Session) (*schemas.Session, error) {
...
func (r *PostgresSessionRepository) Get(ctx context.Context, id uuid.UUID) (*schemas.Session, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || err == sql.ErrNoRows {
			return nil, schemas.ErrSessionNotFound
		}
...
func (r *PostgresSessionRepository) Update(ctx context.Context, id uuid.UUID, entity *schemas.Session) (*schemas.Session, error) {
...
func (r *PostgresSessionRepository) Delete(ctx context.Context, id uuid.UUID) error {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || err == sql.ErrNoRows {
			return schemas.ErrSessionNotFound
		}
...
func (r *PostgresSessionRepository) List(ctx context.Context, limit, offset int32) ([]*schemas.Session, int64, error) {
	...
	items := make([]*schemas.Session, len(results))
...
func mapSessionFromDB(db *Session) *schemas.Session {
	if db == nil {
		return nil
	}

	result := &schemas.Session{
		ID:         db.ID,
		CreatedAt:  db.CreatedAt,
		UpdatedAt:  db.UpdatedAt,
		AuthMethod: db.AuthMethod,
		AuthProvider: func() *schemas.SessionAuthProvider {
			if db.AuthProvider == nil {
				return nil
			}
			v := schemas.SessionAuthProvider(*db.AuthProvider)
			return &v
		}(),
